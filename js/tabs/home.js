...
// live check that the contract numbers are right. Calibration is the one
// exception: its summary uses the tabs' own judgment (js/shared/calSummary.js)
// so it matches what the 검교정 tabs show.
...
async function load() {
  ...
  for (const x of [s, r, l]) if (x.error) throw new Error(x.error.message);
  render(s.data || {}, r.data || [], l.data || []);
}
