...
function urgentListHtml() {
  ...
  const rows = list.map(r => {
    ...
    return `<div class="urg-row" onclick="${click}" title="클릭 → ${repair ? 'EGCS·BWTS 이력' : '업무 탭'}에서 보기">` +
  ...
  return `<div class="urg-box"><h4>🔥 긴급 (상) <span class="muted" style="font-weight:400">${list.length}건 · 📋 업무 탭 긴급 배지 또는 EGCS·BWTS 이력 🔥 로 지정</span></h4>` +
