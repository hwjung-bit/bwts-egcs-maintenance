...
- **세션 TRO 판정(2026-09-01 사용자 기준):** 세션 시작 후 `warmup_minutes`(10분)는 warm-up 제외 → 이후 정상운전 구간의
  **평균**이 5~10ppm이면 정상(범위내 비율 ≥50%·max ≤12면 완화 정상). 정상운전 구간 <3행이면 보류. 10분 넘게 돌렸는데 TRO가
  아예 안 나오면 이탈. 월 판정 = 판정된 발라스트 세션 중 정상 비율 ≥50% (`tro_b_session_ok` = "n/m"). 최솟값은 참고용.
- **밸브 채터링(2026-09-01):** 등급에 반영 안 함. `integrity.flags` 에 "밸브 채터링 N건" 으로 남기고 웹 셀에 ⚙ 표시(스키마 변경 없음). 되돌리려면
  `thresholds.json` `chatter_affects_grade: true`.
- **DataLog 기간 불일치:** 세션 날짜가 전부 폴더 월 밖이면(KMU 7월 폴더에 8월 데이터) 데이터불량 + `datalog_period` 표시.
