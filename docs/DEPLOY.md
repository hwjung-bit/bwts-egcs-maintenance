...
- v20260902d: 💻 PC 폴더 버튼 — `kmtcfolder:bwtslog` 커스텀 프로토콜로 로컬 탐색기 열기.
  PC마다 1회 등록 필요: `scripts/register_kmtcfolder.reg` 더블클릭 (HKCU, 관리자 불필요)
...
## 2026-09-09 WMS 정본 대조 + 선박 WMS 비고 (v20260909c, sql/025)

- 사용자 제공 "KMTC WMS CALIBRATION 현황" 표(9척, TriOS)와 DB 대조 → **40건 갱신**:
  날짜 8건(KNB PAH 2021-08-10, KMU 4건·KCB 3건이 26년 방선분 미반영이었음) + 시리얼 32건.
  KCN WMS1/PH 시리얼은 DB 0690075D → 정본 06901939 로 덮어씀(사용자 확인).
  보류: KMB WMS2/PAH 시리얼 없음(-), KMN WMS2/PAH 시리얼이 엑셀에 `6.00E+45` 로 뭉개져 미입력.
- `sql/025_ships_wms_note.sql` — `ships.wms_note` 신설. 상태 메모(`egcs_wms_memo`)는
  현황 탭에서 상태를 '정상'으로 되돌리면 지워지므로 상시 특이사항을 담을 수 없다.
  선박관리 탭에 'WMS 비고' 칸 추가(셀 클릭 수정), EGCS 검교정 탭 선박 헤더에 📝 + 툴팁, 이력 복사본에 ※ 로 붙는다.
- KDE: WMS 는 GI 인데 초기 제품이라 센서는 TRI-OS 장착. **주기 산정은 GI 기준 유지**(사용자 결정) — 메모로만 표기.
- 셀에서 '일자 미기재'·'모델 미기재' 보조 라벨 제거(v20260909b) — 회색 칸 + 툴팁으로만 구분.
